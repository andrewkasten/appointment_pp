...
import { Button, TextField, Box, List, ListItem, Card, CardContent, Grid, Stack, Typography } from "@mui/material";
import { MapContainer, TileLayer, Marker, Popup } from "react-leaflet";
import axios from "axios";

export default function Appointments({isAuthed}) {
...
function ApptForm() {
  ...
  async function handleSubmit() {
   ...
   await axios.post("http://localhost:8000/api/appointments/", appointmentObject)
  ...
  return (
    ...
    <Card sx = {{maxWidth: 550}}>
      ...
      <Box component="form" sx={{ "& .MuiTextField-root": { m: 0.4, width: "26ch" } }} noValidate autoComplete="off">
        ...
        <TextField label="Notes" value={description} onChange={(e) => setDescription(e.target.value)}/>
      </Box>
 ...
 <h2>Map</h2>
         ...
         {appointments.map((appointment) => (
          <Marker key={appointment.id} position={[appointment.lat, appointment.long]}>
            <Popup>
            {appointment.name}<br/>{appointment.date} <br/>  {appointment.street_1} <br/> {appointment.time}
            </Popup>
            </Marker>
  ...
  return (
    <>
      <Grid container direction="row" spacing={1} sx={{justifyContent: "center", alignItems: "stretch",}}>
        <br></br> {{isAuthed} ? (
          <>           
            <ApptForm />
            <ListAppointments />
